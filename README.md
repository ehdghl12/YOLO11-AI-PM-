...
SAFEKICK는 공유 전동킥보드(Personal Mobility, PM)의 부적절한 반납 문제를 줄이기 위해 설계한 온디바이스 AI 기반 실시간 주차 가이드 시스템이다. 사용자가 반납 사진을 촬영하면 iOS 앱 내부에서 YOLO11 기반 객체 검출 모델과 세그멘테이션 모델이 동작하며, 전동킥보드와 주변 위험 시설물을 분석해 주차 가능 여부를 자동으로 판단한다.

본 프로젝트는 GPS 기반 반납 시스템만으로는 판별하기 어려운 실제 사진 속 위험 객체를 탐지하고, 사용자에게 반납 허용 또는 재주차 유도 결과를 제공하는 것을 목표로 한다.

## 프로젝트 개요

| 항목 | 내용 |
| --- | --- |
| 프로젝트명 | SAFEKICK : YOLO11 기반 온디바이스 AI 실시간 PM 주차 가이드 시스템 |
| 대상 문제 | 공유 PM의 부적절한 반납 및 보행 안전 저해 문제 |
| 핵심 기술 | YOLO11 객체 검출, YOLO11 Segmentation, Core ML 기반 온디바이스 추론 |
| 구현 환경 | iOS 앱 |
| 주요 판단 대상 | 전동킥보드, 맨홀, 점자블록, 소화전, 횡단보도 |
| 최종 출력 | 주차 가능 / 주차 불가능 판정 및 사용자 안내 |

공유 PM 서비스에서는 사용자가 이동을 마친 뒤 앱을 통해 반납을 수행한다. 그러나 실제 현장에서는 점자블록 위, 횡단보도 주변, 소화전 근처, 맨홀 위 등 보행자 안전과 도시 시설물 이용에 방해가 되는 위치에 PM이 반납되는 문제가 지속적으로 발생한다.

SAFEKICK는 반납 사진을 분석하여 PM 주변의 위험 객체를 확인하고, 사전에 정의한 주차 판정 로직에 따라 반납 허용 여부를 결정한다. 모든 AI 추론은 기기 내부에서 수행되므로 서버 전송 없이 빠른 응답이 가능하며, 개인정보 및 네트워크 의존성을 줄일 수 있다.

## 문제 정의

기존 공유 PM 서비스는 대부분 GPS 기반 반납 시스템을 사용한다. GPS는 사용자가 특정 구역 안에 있는지를 확인하는 데에는 효과적이지만, 실제 사진 속 주차 위치가 안전한지 판단하는 데에는 한계가 있다.
...
따라서 본 프로젝트는 단순 위치 정보가 아니라, 사용자가 직접 촬영한 반납 사진을 기반으로 주차 적합성을 판단하는 방식에 주목하였다.
...
| 구분 | 목표 |
| --- | --- |
| 객체 검출 | 반납 사진에서 전동킥보드와 위험 객체를 탐지한다. |
| 영역 분할 | 횡단보도 영역을 Segmentation Polygon으로 검출한다. |
| 주차 판정 | 검출 결과를 바탕으로 주차 가능 / 불가능 여부를 자동 판단한다. |
| 온디바이스 구현 | iOS 앱에서 서버 없이 AI 추론이 수행되도록 구현한다. |
| 사용자 안내 | 판정 결과에 따라 반납 허용 또는 재주차 유도 화면을 제공한다. |

## 데이터셋

모델 학습에는 AI Hub에서 제공하는 안전 위험 시설물 및 도로 시설물 관련 데이터를 사용하였다. 객체 검출 모델에는 BBox 라벨링 데이터를 활용하였고, 횡단보도 탐지를 위한 세그멘테이션 모델에는 Polygon 라벨링 데이터를 활용하였다.

### 데이터 구성

| 클래스 | 라벨 형식 | 사용 목적 |
| --- | --- | --- |
| 전동킥보드 | BBox | PM 위치 및 크기 기준 객체 |
| 맨홀 | BBox | 위험 객체 거리 판정 |
| 소화전 | BBox | 위험 객체 거리 판정 |
| 점자블록 | BBox | 보행 약자 안전 시설물 판정 |
| 횡단보도 | Polygon | 주차 금지 영역 판정 |
